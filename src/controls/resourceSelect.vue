    ...
    computed: {
        ...
        name: {
            get: function() {
                ...
                if (resource != null && resource.name != null) { return resource.name; } else { return "Unknown Resource."; }
            }
        },
        url: {
            get: function() {
                ...
                var resource = this.resource;
                if (resource != null && resource.url != null) { return resource.url; } else { return null; }
            }
        },
        urlTarget: {
            get: function() {
                if (this.uri == null) return null;
                var resource = this.resource;
                if (resource != null && resource.url != null) { return "_blank"; } else { return null; }
            }
        },
        mine: {
            get: function() {
                ...
                var resource = this.resource;
                if (resource != null) {
                    if (resource.owner == null || resource.owner.length === 0) { return true; }
        ...
        description: {
            get: function() {
                if (this.uri == null) return null;
                var resource = this.resource;
                if (resource != null && resource.description != null) { return resource.description; } else { return null; }
            }
        }
    },
    created: function() {
            ...
            repo.searchWithParams(
                ...
                function(actions) {
                    ...
                    for (var i = 0; i < actions.length; i++) {
                        ...
                        if (addresses[action.owner[0]] == null) { addresses[action.owner[0]] = 0; }
                        if (action.type === "LikeAction") { addresses[action.owner[0]] = addresses[action.owner[0]] + 1; }
                        if (action.type === "DislikeAction") { addresses[action.owner[0]] = addresses[action.owner[0]] - 1; }
                    }
                    var upvotes = 0;
                    var downvotes = 0;
                    me.upvoted = false;
                    ...
                    for (var address in addresses) {
                        ...
                        if (value < 0) {
                            downvotes++;
                            if (address === EcIdentityManager.ids[0].ppk.toPk().toPem()) { me.downvoted = true; }
                        } else if (value > 0) {
                            upvotes++;
                            if (address === EcIdentityManager.ids[0].ppk.toPk().toPem()) { me.upvoted = true; }
                        }
                    }
                    me.upvotes = upvotes;
            ...
            repo.searchWithParams(
                ...
                function(actions) {
                    ...
                    for (var i = 0; i < actions.length; i++) {
                        var action = actions[i];
                        if (addresses[action.owner[0]] == null) { addresses[action.owner[0]] = 0; }
                        if (action.type === "ChooseAction") { addresses[action.owner[0]] = addresses[action.owner[0]] + 1; }
                    }
                    var views = 0;
                    me.viewed = false;
                    for (var address in addresses) {
                        ...
                        if (value > 0) {
                            views += value;
                            if (address === EcIdentityManager.ids[0].ppk.toPk().toPem()) { me.viewed = true; }
                        }
                    }
                    me.views = views;
                }, console.error);
