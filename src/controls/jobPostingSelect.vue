<template>
    <li class="jobPostingSelect">
        ...
        <a v-on:click="setJobPosting" href="#" style="cursor:pointer;">
            {{ name }}
            <span v-if="count">({{count}} required skills)</span>
        </a>
...
export default {
    ...
    computed: {
        ...
        selectedJobPosting: {
            get: function() {
                return this.$store.state.selectedJobPosting;
            },
            set: function(val) {
                this.$store.commit("selectedJobPosting", val);
            }
        }
    },
    methods: {
        deleteMe: function() {
            ...
            EcRepository._delete(resource, function() {
                var c = me.selectedJobPosting;
                me.selectedJobPosting = null;
                me.$nextTick(function() {
                    me.selectedJobPosting = c;
                    if (topicCompetencies[me.selectedJobPosting.id] != null)
                        for (var i = 0; i < topicCompetencies[me.selectedJobPosting.id].length; i++)
                            topicCompetencies[me.selectedJobPosting.id][i].getResourceCount();
                });
            }, console.error);
        },
        setJobPosting: function() {
            var me = this;
            this.selectedJobPosting = this.resource;
            repo.precache(this.resource.skills, function() {
                me.$router.push('jobPosting');
            });
        }
    }
};
