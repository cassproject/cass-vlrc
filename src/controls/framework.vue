<template>
    ...
    <span v-if="showButtons" class="buttons btop fright">
        ...
        <button class="inline" v-if="calculate == true" style="color:blue;" v-on:click="calculate = false" title="Turn off processing.">
            <i class="mdi mdi-18px mdi-cogs" aria-hidden="true"></i> Processing</button>
        <button class="inline" v-if="calculate == false" v-on:click="calculate = true" title="Turn on processing, showing estimated ability.">
            <i class="mdi mdi-18px mdi-cogs" aria-hidden="true"></i> Processing</button>
    </span>
...
export default {
    ...
    computed: {
        ...
        computedState: function() {
            var me = this;
            if (this.calculate === false) { return 0; }
            if (!this.once) return;
                ...
                new EcAsyncHelper().each(app.assertions, function(a, done) {
                    if (a == null) { done(); } else {
                        a.getSubjectAsync(function(subject) {
                            if (subject.toPem() === me.subject) { assertions.push(a); }
                            done();
                        }, done);
                    }
                }, function(as) {
                    ...
                    frameworkGraph.addFramework(EcFramework.getBlocking(me.uri), repo, function() {
                        frameworkGraph.processAssertionsBoolean(assertions, function() {
                            if (hash === me.computedStateAssertionLength + app.assertionsChanges + me.uri) {
                                for (var key in frameworkGraph.metaVerticies) { delete frameworkGraph.metaVerticies[key].framework; }
                                me.computedStateActual = frameworkGraph;
                                console.log("Finished processing (overwrite): " + new Date() + " " + hash);
                            } else { console.log("Finished processing (abort): " + new Date() + " " + hash); }
                        }, console.error);
                    }, console.error);
                });
            }
