<template>
    ...
    <ul v-if="people" >
    <profile v-for="item in people" :key="item.id" :pk="item.owner[0]" :displayName="item.name" :onClick="changeSelected"></profile>
    </ul>
        ...
        people: {
            get: function() {
                ...
                for (var i = 0; i < app.people.length; i++) {
                    if (app.people[i].owner == null || app.people[i].length === 0) { continue; }
                    if (EcPk.fromPem(app.people[i].owner[0]).fingerprint() !== app.people[i].getGuid()) { continue; }
                    if (app.people[i].getName() == null) { continue; }
                    if (this.search === "" || app.people[i].getName().toLowerCase().indexOf(this.search.toLowerCase()) !== -1) { results.push(app.people[i]); }
                }
                return results;
            }
        }
