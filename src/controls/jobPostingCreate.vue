<template>
    <div class="jobPostingCreate"><hr><h3>New Posting</h3>
        <select v-model="type">
        ...
        <ul>
            <div v-for="(item,index) in skills" :key="item">
                <button v-on:click="removeRequirement(index);">Remove</button>
                <competency :uri="item" :parentCompetent="true" />
            </div>
        </ul>
        <ul>
            <div v-for="(item,index) in candidates" :key="item">
                <button v-on:click="removeCandidate(index);">Remove</button>
                <input :value="item" class="autoSuggest" />
            </div>
        </ul>
        <label for="tags">Add Requirements: </label>
        <input type="text" class="tags" v-model="candidateRequirement">
        <button v-on:click="saveNewPosting">Create New Posting</button>
        <button v-on:click="readFromClipboard">Read from Clipboard</button>
    </div>
</template>
<script>
export default {
